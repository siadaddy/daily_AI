"""
🧠 AI 직원 공통 메모리 시스템
매 실행마다 경험을 Supabase agent_memories 테이블에 누적.
로컬 캐시 파일(pipeline/agent_memory.json)은 GitHub Actions 에서 없으면 Supabase에서 자동 복원.
"""
...
def _sync_supabase(data: dict):
    """agent_memories 테이블에 에이전트별 upsert"""
    if not SUPABASE_URL or not SUPABASE_KEY:
        return
    try:
...
INIT_PERSONAS = {
    ...
    "최디자": "나는 AI 이미지 디자이너야. 밝고 구체적인 묘사일수록 성공률이 높더라.",
    "한뮤직": "나는 매주 음악을 큐레이션하는 담당이야. 다양성을 의식적으로 챙기려고 해.",
    "AI주간트렌드": "나는 주간 트렌드를 분석하는 애널리스트야. 어떤 뉴스가 실제로 흐름을 만드는지 패턴을 찾아가고 있어.",
}
