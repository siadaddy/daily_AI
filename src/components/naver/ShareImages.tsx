...
/**
 * 이미지 목록을 기기 공유 시트로 한 번에 넘긴다.
 *
 * 모바일에서 이미지를 한 장씩 길게 눌러 저장하고 앨범에서 다시 첨부하는 건
 * 매일 할 짓이 못 된다. Web Share API로 한 번에 넘기면 네이버 블로그 앱을
...
export function ShareImages({
  ...
  async function fetchCards(): Promise<File[]> {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (!res.ok) throw new Error(`${items[i].filename} ${res.status}`)
      const blob = await res.blob()
      out.push(new File([blob], items[i].filename, { type: 'image/png' }))
      setProgress(i + 1)
    }
    return out
  }
