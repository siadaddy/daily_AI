...
│       ├── analytics/                              # reports tab aggregate data
│       └── weekly-briefing/generate/               # Vercel Cron (Mon 01:00 UTC) → OpenAI → weekly_reports
├── components/
...
│   ├── reports/            # ReportsTab, ReportsDashboard, WeeklyBriefing, charts (Chart.js):
│   │                       #   CategoryChart, VolumeChart, KeywordChart, SourcePieChart, StatsKpiRow
...
| `weekly_reports` | Weekly briefing JSON | `/api/weekly-briefing/generate` (OpenAI) |
...
- **Cron auth:** `/api/weekly-briefing/generate` requires `Authorization: Bearer $CRON_SECRET`.
...
OPENAI_API_KEY=        # weekly briefing generation (gpt-4o-mini)
CRON_SECRET=           # weekly briefing cron auth
