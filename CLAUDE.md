...
- **Four type roles, no overlap.** Song Myung (`--font-display`) for headlines 28px and up — it is a high-contrast 명조 that only ships weight 400, so never bump its `font-weight` (the browser fakes a bold and the thin horizontals break up); Gowun Batang (`--font-serif`) for serif text under 28px, Noto Sans KR (`--font-sans`) for Korean body copy, IBM Plex Mono (`--font-mono`) for kickers, dates and every number — numeric UI also needs `font-variant-numeric: tabular-nums`. Use `.ed-headline` for the display face and `.ed-display` for the text serif — picking the wrong one at the wrong size is the main way to make this look off. Deliberately **not** a Noto superfamily: Noto Sans/Serif KR share a skeleton, so headline and body read as one voice at two sizes. Two consequences to respect: Korean 바탕체 has no true italic (never apply `italic` to `--font-serif` — the browser fakes a slant), and negative letter-spacing is a Latin-sans habit that crowds Hangul, so display type stops at `-0.01em`. IBM Plex Mono carries no Hangul, which is why `--font-mono` falls through to Noto Sans KR before generic `monospace`.
